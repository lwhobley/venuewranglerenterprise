...
export type WranglerAction = { id: string; type: 'NAVIGATE' | 'ACKNOWLEDGE' | 'REASSIGN_RESERVATION'; label: string; route: '/reservations' | '/staff' | '/bar-stock' | '/reports' | '/floor'; requiresConfirmation: boolean; payload?: Record<string, string | number | boolean | null> };
...
export type WranglerSummary = { covers: number; reservations: number; vipArrivals: number; lowStockItems: number; eightySixItems: number; pendingStaffRequests: number; seatedTables: number };
...
const EMPTY_WRANGLER_SUMMARY: WranglerSummary = {
  covers: 0, reservations: 0, vipArrivals: 0,
