...
const mocks = vi.hoisted(() => ({ request: vi.fn(), data: {} as Record<string, unknown>, invalidate: vi.fn(), params: {} as Record<string, string> }));
...
describe('reconciliation screens', () => {
  it('opens the exact stand requested by the stadium map', async () => {
    mocks.params = { standCode: 'STAND-2' };
    mocks.data['/v1/stadium/concourse/stand-sheets'] = [
      { id: 'sheet-1', outlet: { name: 'Stand 1', code: 'STAND-1' }, status: 'count_in_recorded', inventoryVariance: null, countIn: [{ code: 'water', name: 'Water', count: 10, unitPriceCents: 200 }] },
      { id: 'sheet-2', outlet: { name: 'Stand 2', code: 'STAND-2' }, status: 'count_in_recorded', inventoryVariance: null, countIn: [{ code: 'pretzel', name: 'Pretzel', count: 8, unitPriceCents: 600 }] },
    ];

    await act(async () => root.render(<StandSheet />));

    expect(container.querySelector('input[aria-label="Pretzel Count out"]')).not.toBeNull();
    expect(container.querySelector('input[aria-label="Water Count out"]')).toBeNull();
  });
