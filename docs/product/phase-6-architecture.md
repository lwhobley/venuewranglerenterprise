...
| Hospitality requests and fulfillment | Previously loaded orders are available from the encrypted, identity-and-capability-scoped cache for up to 12 hours after transport failure. | Request drafts are encrypted locally and queued; kitchen preparation, per-line fulfillment batches, pickup, rejection, and cancellation are online-only. | Queued requests retain their event and identity scope and sync when connectivity returns. Kitchen actions always require current server state. Fulfillment batches are append-only, tenant-scoped records; the database caps cumulative delivered quantity at the requested amount. Substitutions and partial deliveries require reasons. |
...
| Event closeout | Previously loaded closeout review is cached for up to 12 hours after transport failure. | Outcome-summary notes can be saved to the encrypted, identity-and-capability-scoped device outbox. Exception decisions, finalization, and post-close correction addenda require connectivity. | Pending notes sync only while the event remains open; server conflicts keep the local note and mark it for review. The server rechecks live exceptions during finalization, stores immutable audit history, blocks ordinary event mutations after closure, and permits only append-only audited correction records. |
