...
| --- | --- | --- | --- | --- |
| Tenant isolation and runtime database role | PostgreSQL RLS and FORCE RLS; health gate checks 45 protected tables, including hospitality handoff receipts; CI provisions separate `venue_migrator` and `venue_app` roles and runs tenant-isolation probes. | Repository updated; confirm the 45-table gate on the next CI run | Approved staging health response, runtime role attributes, full tenant-isolation run, deployed commit SHA. | Cloud operator / security reviewer |
