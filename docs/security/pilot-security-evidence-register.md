...
| --- | --- | --- | --- | --- |
| Tenant isolation and runtime database role | PostgreSQL RLS and FORCE RLS; health gate checks 45 protected tables, including hospitality handoff receipts; CI provisions separate `venue_migrator` and `venue_app` roles and runs tenant-isolation probes. | CI verified on [run 36142501235](https://github.com/lwhobley/venuewranglerenterprise/actions/runs/36142501235) for commit `dee5be7` on 2026-09-25. Staging evidence is still required. | Approved staging health response, runtime role attributes, full tenant-isolation run, deployed commit SHA. | Cloud operator / security reviewer |
