...
| --- | --- | --- | --- | --- |
| Tenant isolation and runtime database role | PostgreSQL RLS and FORCE RLS; health gate checks 43 protected tables; CI provisions separate `venue_migrator` and `venue_app` roles and runs tenant-isolation probes. | CI verified on clean PostgreSQL 17 | Approved staging health response, runtime role attributes, full tenant-isolation run, deployed commit SHA. | Cloud operator / security reviewer |
