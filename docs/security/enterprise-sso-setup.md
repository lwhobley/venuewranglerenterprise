...
Supported capabilities also include `operations:read`, `operations:write`, `notification:read`, `issue:evidence`, and `tenant:admin`. Grant `notification:read` only to users allowed to receive in-app notifications. Notifications are additionally isolated by the verified tenant and actor subject in PostgreSQL RLS. Grant `issue:evidence` only to users allowed to upload issue photos. `tenant:admin` remains a separate, privileged capability for venue onboarding.
