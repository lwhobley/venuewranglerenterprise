...
Supported capabilities also include `operations:read`, `operations:write`, `event:closeout`, `hospitality:order`, `hospitality:fulfill`, `notification:read`, `issue:evidence`, and `tenant:admin`. Grant `event:closeout` only to event operations managers or explicitly delegated closeout owners; it requires the event scope, and follow-up assignment is additionally checked against the token's assignable-user list. Finalization rechecks all current exceptions and database triggers reject ordinary event writes after closure. Grant `hospitality:order` to authorized event service requesters; grant `hospitality:fulfill` only to kitchen/service operators who may advance order preparation and handoff. Both capabilities still require event, venue, and location scope. Grant `notification:read` only to users allowed to receive in-app notifications. Notifications are additionally isolated by the verified tenant and actor subject in PostgreSQL RLS. Grant `issue:evidence` only to users allowed to upload issue photos. `tenant:admin` remains a separate, privileged capability for venue onboarding.
