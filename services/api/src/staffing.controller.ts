...
export class StaffingController {
  ...
  @Post('events/:eventId/shifts/:shiftId/attendance/correction') correctAttendance(@Req() req: Request, @Param('eventId', new ParseUUIDPipe()) eventId: string, @Param('shiftId', new ParseUUIDPipe()) shiftId: string, @Body() dto: StaffAttendanceCorrectionDto, @Headers('idempotency-key') key: string) {
    return this.staffing.correctAttendance(req.identity, eventId, shiftId, dto, this.key(key));
  }
