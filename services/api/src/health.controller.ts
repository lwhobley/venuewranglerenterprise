    ...
    try {
      ...
      const [tables] = await this.prisma.$queryRaw<Array<{ protected: boolean; table_count: bigint; policy_table_count: bigint }>>`
        SELECT bool_and(c.relrowsecurity AND c.relforcerowsecurity) AS protected,
               ...
               (SELECT count(DISTINCT p.tablename) FROM pg_policies p WHERE p.schemaname = 'public' AND p.tablename IN ('organizations','venues','events','locations','issues','issue_audit_events','command_receipts','issue_events')) AS policy_table_count
        FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname = 'public' AND c.relname IN ('organizations','venues','events','locations','issues','issue_audit_events','command_receipts','issue_events')
      `;
      const rlsEnforced = role !== undefined && !role.rolsuper && !role.rolbypassrls && tables?.protected === true && tables.table_count === 8n && tables.policy_table_count === 8n;
