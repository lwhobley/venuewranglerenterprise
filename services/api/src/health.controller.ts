...
export class HealthController {
  ...
  async check(@Res() response: Response) {
    try {
      ...
      const [tables] = await this.prisma.$queryRaw<Array<{ protected: boolean; table_count: bigint; policy_table_count: bigint }>>`
        SELECT bool_and(c.relrowsecurity AND c.relforcerowsecurity) AS protected,
               ...
               (SELECT count(DISTINCT p.tablename) FROM pg_policies p WHERE p.schemaname = 'public' AND p.tablename IN (
                 ...
                 'stock_items','stock_counts','stock_count_lines','stock_movements','stock_count_audit','hospitality_orders','hospitality_order_lines','hospitality_order_audit','stock_transfers','stock_transfer_lines','stock_transfer_audit',
                 'event_closeouts','event_closeout_followups','event_closeout_audit','staffing_vendor_requests','staffing_vendor_request_audit'
      ...
      `;
      const rlsEnforced = role !== undefined && !role.rolsuper && !role.rolbypassrls && tables?.protected === true && tables.table_count === 42n && tables.policy_table_count === 42n;
